  ...
  for (const [name, values] of Object.entries(filters.select)) {
    ...
    chips.push({
      ...
      onToggle: (id) =>
        setFilters((p) => ({
          ...p,
          select: { ...p.select, [name]: toggle(p.select[name] ?? [], id) },
        })),
    ...
    <div className="filter-bar">
      {chips.map((c) => (
        <span key={c.key} className="chip">
          ...
          >
            <button>
              {c.values.length > 2 ? `${c.values.length} selected` : c.values.join(', ')}
            </button>
          </Picker>
        ...
        onSelect={(id) => {
          if (picking === ASSIGNEE)
            ...
            setFilters((p) => ({
              ...
              select: {
                ...p.select,
                [pickingField.name]: toggle(p.select[pickingField.name] ?? [], id),
              },
            }));
        }}
      >
          ...
          <Menu>
            ...
            <MenuContent align="end">
              ...
              {(['active', 'open', 'closed', 'all'] as StateFilter[]).map((s) => (
                <MenuItem
                  key={s}
                  onSelect={() => setFilters((p) => ({ ...p, state: s }))}
                  hint={
                    s === filters.state ? '✓' : s === 'closed' ? String(closedCount) : undefined
                  }
                >
                  {STATE_LABEL[s]}
              ...
              <MenuItem
                onSelect={() =>
                  setFilters((p) => ({
                    ...p,
                    archived: !p.archived,
                    state: p.archived ? 'active' : 'all',
                  }))
                }
                hint={filters.archived ? '✓' : undefined}
              >
                Archived
              </MenuItem>
              {n > 0 && (
