...
/**
 * Email code sign-in: `signInWithOtp` sends a 6-digit code (the Supabase email templates are
 * customized to show `{{ .Token }}`, not the link) and `verifyOtp` checks what they type back
 * in on this page. `profiles.allowed` (set by the `handle_new_user` trigger from the email
 * domain) is the actual gate, checked server-side by requireUser() on every protected route.
 */
